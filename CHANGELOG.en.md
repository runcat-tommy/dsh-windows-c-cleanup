...
- `npm run m5:client` gains section 8 (8 checks, the confirm gate) and section 9 (6 checks, locale following): all five gate states pinned, a visible reason on first render, all four reason strings present in both languages, the hint styled as small secondary text; and one notice object rendering differently per locale, no Han in English with placeholders still substituted, the extra sentence appended in the current language, every `setNotice` passing a key + params, and `scan-view` being called when the locale changes.
