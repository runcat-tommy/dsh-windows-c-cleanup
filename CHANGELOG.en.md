...
- **Every block in the panel now carries a module-name label** (new): Overview / Trend / Controls (scan → select → run) / Status & notices / Cleanup candidates (🟢/🟡/🟠/🔴) / Long-term protections / Preview result / Run progress / Migration preview / Records & artifacts — so you can point at a block by name. Both READMEs gained a matching "module map" table, and checks 10.1–10.6 keep the labels and the docs aligned (including "the docs must list every module"). The label is an 11px rounded pill with a `data-module` attribute — a label, not a heading, so it never competes with content; the three dynamic blocks (preview / progress / migration) now show the stable name first and their dynamic sentence after it.
- **Flow arrows between the groups**: a right-pointing `→` follows the "Scan" button and the "Clear selection" button, spelling out "scan first → then select → finally run". Each arrow carries 12px of margin on both sides (on top of the group's 6px gap) and is `aria-hidden` + `pointer-events:none` — decoration only, skipped by screen readers and never swallowing clicks (checks 5.16, 5.17, 7.9).
...
- `npm run m5:client` gains section 8 (8 checks, the confirm gate), section 9 (6 checks, locale following) and section 10 (6 checks, module names): all five gate states pinned, a visible reason on first render, all four reason strings present in both languages, the hint styled as small secondary text; and one notice object rendering differently per locale, no Han in English with placeholders still substituted, the extra sentence appended in the current language, every `setNotice` passing a key + params, and `scan-view` being called when the locale changes; plus all 10 module names present in both languages with no Han in the English ones, the always-on labels appearing exactly once in position order, no empty headings when there is no data, both READMEs listing every module name, and the label styled as a small pill rather than a heading.
