...
- **组间流向箭头**：「扫描」按钮后与「清空选择」按钮后各加一个向右箭头 `→`，标明「先扫描 → 再选择 → 最后执行」。箭头左右各留 12px（叠上组内 6px gap，留白明显），`aria-hidden` + `pointer-events:none`：纯装饰，读屏跳过、不抢点击（测试 5.16、5.17、7.9）。
...
- `npm run m5:client` 新增第 8 节（8 项，确认门禁）与第 9 节（6 项，语言跟随）：五个门禁状态逐一钉住、首屏渲染出可见原因、四种原因文案中英齐备、提示样式是小字次要色；同一提示对象中英不同、英文无中文且参数照旧替换、附加句按语言拼接、`setNotice` 全部传「键 + 参数」、语言变化时会调 `scan-view` 重取视图。
