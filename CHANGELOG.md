...
- **面板每个区块都挂上「功能区名」标签**（新）：概览 / 历史对比 / 操作区（扫描 → 选择 → 执行）/ 状态与提示 / 清理候选（🟢/🟡/🟠/🔴）/ 长期防护 / 预演结果 / 执行进度 / 迁移预览 / 记录与产物 —— 用户可以直接指着名字说事，README 双语各有一张「功能区对照表」，名字与界面由测试 10.1–10.6 对齐（含"文档里必须列全"这条）。标签做成 11px 圆角小 pill + `data-module` 属性，是标签不是大标题，不抢内容视线；三个动态区块（预演/进度/迁移）原本只有一句动态标题，现在稳定名字在前、动态内容在后。
...
- `npm run m5:client` 新增第 8 节（8 项，确认门禁）、第 9 节（6 项，语言跟随）与第 10 节（6 项，功能模块名）：五个门禁状态逐一钉住、首屏渲染出可见原因、四种原因文案中英齐备、提示样式是小字次要色；同一提示对象中英不同、英文无中文且参数照旧替换、附加句按语言拼接、`setNotice` 全部传「键 + 参数」、语言变化时会调 `scan-view` 重取视图；10 个模块名中英齐备且英文无中文、常驻模块标签按位置各出现一次、无数据时不空挂标题、README 双语必须列全模块名、标签样式是小 pill 而不是大标题。
