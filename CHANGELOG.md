...
- **「预演」旁边的 `?` 说明按钮**：就地解释预演是什么（真跑安全闸与逐项计划、但不动任何数据），并明确三条硬边界（逐项确认、拒绝项永不可执行、除提权任务外不做任何提权动作）。
...
- `npm run m5` 新增第 10 节（10 项）：英文文案覆盖率（101 条规则 + 6 条长期防护全覆盖、id 严格对齐）、英文里不得出现中文、同一 id 两种语言不同、默认语言仍是中文、缺项回退、安全闸英文拒绝理由、RPC 端点 locale 透传、本次扫描到的规则是否全部有英文版本。
- `npm run m5:client` 新增第 5 节（15 项）：中英字典键集合严格一致、英文值无中文、占位符替换、插槽声明了 locale 命名空间、tab 标题是 thunk、中英两种环境各真渲染一次（英文界面不得残留中文）、无 `t` 席位时也能渲染、每次调用都带语言、工具栏三组与按钮顺序、问号说明按钮存在且带展开状态。
